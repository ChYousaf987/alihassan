...
const Skills = () => {
  ...
  return (
    <div id="skills" className="py-10 text-center text-white flex flex-col items-center px-4 sm:px-6 md:px-8">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      <div className="flex flex-wrap justify-center space-x-2 sm:space-x-4 md:space-x-8 mb-6">
        {Object.keys(skillsData).map((category) => (
          <button
            ...
            className={`px-3 sm:px-4 md:px-6 py-2 text-sm sm:text-base md:text-lg font-semibold rounded-md transition-all duration-300 transform hover:scale-110 ${
      ...
      <motion.div
        ...
        transition={{ duration: 0.5 }}
        className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 w-full max-w-6xl"
      >
        {skillsData[selectedCategory].map((skill, index) => (
          <div
            key={index}
            className="flex-shrink-0 relative w-full h-44 flex flex-col items-center justify-center bg-gray-900 rounded-lg shadow-lg border-2 border-gray-800 hover:border-purple-700 hover:scale-105 transition-all duration-300"
          >
